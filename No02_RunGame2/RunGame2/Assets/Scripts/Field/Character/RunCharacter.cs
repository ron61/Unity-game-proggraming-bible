...
public class RunCharacter : MonoBehaviour
{
	...
	{
		const float maxX = 4f; //射撃範囲左右のハーフサイズ
		const float maxY = 5f; //射撃範囲上下のハーフサイズ

		var toTarget = shootTarget - CenterPosition;
		var toWall = Vector3.Dot(toTarget, Vector3.forward) * Vector3.forward;

		// 壁沿いの位置を計算(中心位置を原点にする)
		var subVec = toTarget - toWall;

		// 位置を左右成分、上下成分に分解する
		float distX = Vector3.Dot(subVec, Vector3.right);
		float distY = Vector3.Dot(subVec, Vector3.up);

		// 射撃範囲の割合に変換してAnimator(内のブレンドツリー)に渡す
		animator.SetTrigger("Shoot");
		animator.SetFloat("ShootX", Mathf.Clamp(distX / maxX, -1f, 1f));
		animator.SetFloat("ShootY", Mathf.Clamp(distY / maxY, -1f, 1f));
	}
}
